...
public class APPlacements {
    ...
    public static final ResourceKey<PlacedFeature> BURIED_METEORITE = createKey("buried_meteorite");
   // public static final ResourceKey<PlacedFeature> LARGE_CRATER = createKey("large_crater");

    public static void bootstrap(BootstapContext<PlacedFeature> context) {
        HolderGetter<ConfiguredFeature<?, ?>> configuredFeatureGetter = context.lookup(Registries.CONFIGURED_FEATURE);

        register(context, OVERWORLD_METEORITE, configuredFeatureGetter.getOrThrow(APFeatures.CONFIGURED_OVERWORLD_METEORITE), Arrays.asList(RarityFilter.onAverageOnceEvery(150), InSquarePlacement.spread(), PlacementUtils.HEIGHTMAP_OCEAN_FLOOR, BiomeFilter.biome()));
        register(context, BURIED_METEORITE, configuredFeatureGetter.getOrThrow(APFeatures.CONFIGURED_BURIED_METEORITE), Arrays.asList(RarityFilter.onAverageOnceEvery(100), InSquarePlacement.spread(), PlacementUtils.HEIGHTMAP_OCEAN_FLOOR, BiomeFilter.biome()));
