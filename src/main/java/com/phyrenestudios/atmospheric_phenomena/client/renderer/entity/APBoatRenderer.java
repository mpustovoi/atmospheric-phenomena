...
public class APBoatRenderer extends EntityRenderer<APBoat> {
    ...
    public APBoatRenderer(EntityRendererProvider.Context context, boolean chest) {
        super(context);
        this.shadowRadius = 0.8F;
        this.boatResources = Stream.of(APBoat.Type.values()).collect(ImmutableMap.toImmutableMap(type -> type, type -> Pair.of(new ResourceLocation(AtmosphericPhenomena.MODID, getTextureLocation(type, chest)), this.createBoatModel(context, type, chest))));
    }

    private BoatModel createBoatModel(EntityRendererProvider.Context context, APBoat.Type type, boolean chest) {
    ...
    private static ModelLayerLocation createLocation(String path) {
        return new ModelLayerLocation(new ResourceLocation(AtmosphericPhenomena.MODID, path), "main");
    }
    ...
    private static String getTextureLocation(APBoat.Type type, boolean chest) {
        return chest ? "textures/entity/chest_boat/" + type.getName() + ".png" : "textures/entity/boat/" + type.getName() + ".png";
    }

    @Override
    public void render(APBoat boat, float boatYaw, float partialTicks, PoseStack stack, MultiBufferSource buffer, int light) {
        stack.pushPose();
    ...
    @Override
    public ResourceLocation getTextureLocation(APBoat boat) {
        return this.getModelWithLocation(boat).getFirst();
    }

    public Pair<ResourceLocation, BoatModel> getModelWithLocation(APBoat boat) {
        return this.boatResources.get(boat.getAPBoatType());
    }
}
